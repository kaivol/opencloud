...
type Config struct {
	...
	ClientID     string     `yaml:"client_id" env:"OC_OIDC_CLIENT_ID;WEBFINGER_OIDC_CLIENT_ID" desc:"The OIDC client ID for clients. This client ID will be provided to clients via webfinger properties." introductionVersion:"%%NEXT%%"`
	Scope        string     `yaml:"scope" env:"OC_OIDC_SCOPE;WEBFINGER_OIDC_SCOPE" desc:"OIDC scopes to request during authentication to authorize access to user details. Defaults to 'openid profile email'. Values are separated by blank. More example values but not limited to are 'address' or 'phone' etc." introductionVersion:"%%NEXT%%"`
